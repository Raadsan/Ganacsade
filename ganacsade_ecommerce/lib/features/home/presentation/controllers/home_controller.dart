...
class HomeController extends GetxController {
  ...
  final AdvertisementsApiService _advertisementsApiService = AdvertisementsApiService();
  final DataPackagesApiService _dataPackagesApiService = DataPackagesApiService();
  
  // Observable variables
  final RxList<PromotionalBanner> promotionalBanners = <PromotionalBanner>[].obs;
      ...
      if (adsData.isNotEmpty) {
        ...
        promotionalBanners.value = adsData.map((json) {
          ...
          if (imageUrl.startsWith('/uploads')) {
            imageUrl = '${ApiConfig.getBaseUrl().replaceAll('/api', '')}$imageUrl';
          }
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      final totalAdminProducts = adminCats.fold<int>(0, (sum, cat) => sum + cat.productCount);
      ...
      categories.value = [
        StaticDataPackagesCategory.getCategory().copyWith(productCount: dataPackagesCount),
        categoriesMetaCategory,
      ];
      
      print('✅ Loaded 2 main categories: Internet Services + Categories (${adminCats.length} subcategories)');
    } catch (e) {
      ...
      categories.value = [StaticDataPackagesCategory.getCategory().copyWith(productCount: dataPackagesCount)];
    }
  }
  ...
  Future<void> _loadFlashSaleProducts() async {
    try {
      ...
      if (productsData.isNotEmpty && productsData[0]['flash_end_time'] != null) {
        try {
          flashSaleEndTime.value = DateTime.parse(productsData[0]['flash_end_time']);
  ...
  void _startBannerAutoScroll() {
    _bannerTimer = Timer.periodic(const Duration(seconds: 4), (timer) {
      if (promotionalBanners.isNotEmpty) {
        final nextIndex = (currentBannerIndex.value + 1) % promotionalBanners.length;
  ...
  void _startFlashSaleTimer() {
    _flashSaleTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      final endTime = flashSaleEndTime.value ?? DateTime(now.year, now.month, now.day + 1);
  ...
  Future<void> refreshData() async {
    ...
    try {
      ...
      Get.snackbar(
        'Refresh Failed',
        'Could not refresh data. Please check your connection.',
        snackPosition: SnackPosition.BOTTOM,
        duration: const Duration(seconds: 3),
        backgroundColor: const Color(0xFFF44336),
        colorText: const Color(0xFFFFFFFF),
  ...
  CategoryType _getCategoryTypeFromName(String name) {
    ...
    } else if (nameLower.contains('electronic') || nameLower.contains('elektaroon')) {
      return CategoryType.electronics;
    } else if (nameLower.contains('men') || nameLower.contains('ragga')) {
      return CategoryType.mens;
    } else if (nameLower.contains('women') || nameLower.contains('haweenka') || nameLower.contains('dumarka')) {
    ...
    } else if (nameLower.contains('good') || nameLower.contains('alaabta') || nameLower.contains('general')) {
